...
def get_joint_position(locobot, link_id):
...
class CameraController:
    ...
    def scan_room(self):
        """Move camera in a pattern to scan the room."""
        # Define scanning pattern
        scan_positions = [
            {"pan": -1.2, "tilt": -0.1},  # Far left
            {"pan": -0.6, "tilt": -0.2},  # Mid left
            {"pan": 0.0, "tilt": -0.3},   # Center
            {"pan": 0.6, "tilt": -0.2},   # Mid right
            {"pan": 1.2, "tilt": -0.1},   # Far right
        ]

        original_pan = self.pan_target
        original_tilt = self.tilt_target

        try:
            for pos in scan_positions:
                # Calculate relative movement needed
                pan_delta = pos["pan"] - self.pan_target
                tilt_delta = pos["tilt"] - self.tilt_target

                # Move camera
                self.move_camera(pan_delta=pan_delta, tilt_delta=tilt_delta)

                # Wait for camera to settle
                import time
                time.sleep(1.0)

                # Yield control back to caller (useful if this is used in a generator)
                yield self.pan_target, self.tilt_target
        finally:
            # Return to original position
            self.move_camera(
                pan_delta=original_pan - self.pan_target,
                tilt_delta=original_tilt - self.tilt_target
            )

    def test_direct_camera_control(self):
        """Force camera movement with direct joint position control."""
        print("\n=== DIRECT CAMERA CONTROL TEST ===")

        if "pan" not in self.dof_map or "tilt" not in self.dof_map:
            print("ERROR: Camera joints not found in dof_map")
            return False
        ...
        pan_pos = get_joint_position(self.locobot, pan_id)
        tilt_pos = get_joint_position(self.locobot, tilt_id)
        print(f"Initial camera position: Pan={pan_pos:.3f}, Tilt={tilt_pos:.3f}")

        # Try setting joint positions directly
        try:
            # Set extreme position through joint positions
            ...
            if pan_offset >= 0 and tilt_offset >= 0:
                ...
                joint_positions[pan_offset] = 1.0
                joint_positions[tilt_offset] = -0.5
                self.locobot.joint_positions = joint_positions
                print("Forced camera to extreme right and down position")
                ...
                new_pan_pos = get_joint_position(self.locobot, pan_id)
                new_tilt_pos = get_joint_position(self.locobot, tilt_id)
                print(f"Final camera position: Pan={new_pan_pos:.3f}, Tilt={new_tilt_pos:.3f}")

                # Verify movement
                if abs(new_pan_pos - pan_pos) < 0.1 and abs(new_tilt_pos - tilt_pos) < 0.1:
                    print("WARNING: Camera did not move significantly!")
                else:
                    print("Camera movement succeeded!")

                return True
        except Exception as e:
            print(f"ERROR in direct camera control: {e}")
            import traceback
            traceback.print_exc()
